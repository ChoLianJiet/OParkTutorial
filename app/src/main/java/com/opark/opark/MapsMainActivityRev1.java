...
public class MapsMainActivityRev1 extends FragmentActivity implements OnMapReadyCallback,GoogleApiClient.ConnectionCallbacks,
...
//    String LOCATION_PROVIDER = LocationManager.GPS_PROVIDER;
    ...
    public static DatabaseReference matchmakingRef;
...
//        matchmakingRef.child(foundUser).removeValue();
...
//                Log.d("oldArrayList", "oldArrayList consists" + oldArrayList);
                ...
                if (oldArrayList.contains(key)) {
                    ...
                    Log.d("OPark", "this is myself "+ (currentUserID));
                    newArrayList.remove(key);
                    newHashSet.remove(key);
                    oldArrayList.remove(key);
                    oldHashSet.remove(key);
                    Log.d("Self", "removed self new array = " + newArrayList);
                    Log.d("Self", "removed self old array = " + oldArrayList);

                }   else {
                    if (!currentUserID.equals(key) && !oldArrayList.contains(key)) {

                        Log.d("OPark", "add " + (key) + " to newArrayList");
                        newArrayList.add(key);
                        newHashSet.addAll(newArrayList);
                        newArrayList.clear();
                        newArrayList.addAll(newHashSet);
                        Log.d("OPark", key + " Added to newArraylist = " + newArrayList);
                        Log.d("OPark", "oldArraylist = " + oldArrayList);
//
...
//                        Log.d("OPark", " Loop  " + i + " newArrayList " + newArrayList);
//                        Log.d("OPark","Loop  " + i + " oldArrayList " + oldArrayList);

                            matchmakingRef.child(foundUser).addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(DataSnapshot dataSnapshot) {
                                    try {
                                        Log.d("OPark", "\nonDataChange first line, newArrayList =  " + newArrayList);
                                        ...
                                        if ((newArrayList != null) && (adatemValue.equals(ADATEM0))) {
                                            if (newArrayList.get(0).equals(matchmakingRef.child(newArrayList.get(0)).getKey())) {


//                                    if (adatemValue.equals(ADATEM0)) {
                                                //Setting adatem0 to 1

                                                Log.d("OPark", "Found adatem 0, foundUser is " + foundUser);
                                                matchmakingRef.child(newArrayList.get(0)).child("adatem").setValue("1");
                                                Log.d("OPark", "matchmakingRef foundUser is" + matchmakingRef.child(newArrayList.get(0)).getKey());

                                                loadingCircle.setVisibility(View.INVISIBLE);
                                                Intent intent = new Intent(MapsMainActivityRev1.this, UserPopUp.class);
...
//
//                                            Intent intent2 = new Intent(MapsMainActivityRev1.this, NoUserPopUp.class);
//                                            startActivity(intent2);
//
//                                            loadingCircle.setVisibility(View.INVISIBLE);
//                                        }

                                        matchmakingRef.child(foundUser).removeEventListener(this);
...
//                }
                ...
                if (newArrayList.isEmpty()){
                    ...
                    Log.d("OPark", " arraylist is empty \nadd old array list to new, now old is " + oldArrayList);
                }

                if (foundUser == null) {
                    Log.d("NoUser","No Found users");
                    Intent intent2 = new Intent(MapsMainActivityRev1.this, NoUserPopUp.class);
                    startActivity(intent2);
                    loadingCircle.setVisibility(View.INVISIBLE);
                } else {
...
//            markerCount=1;
    ...
    public static void animateMarker(final Location destination, final Marker marker) {
        if (marker != null) {
            ...
            final MapsMainActivityRev1.LatLngInterpolator latLngInterpolator = new MapsMainActivityRev1.LatLngInterpolator.LinearFixed();
    ...
    private interface LatLngInterpolator {
        ...
        class LinearFixed implements MapsMainActivityRev1.LatLngInterpolator {
            @Override
    ...
    private void signOut(){
        ...
        Intent intent = new Intent(MapsMainActivityRev1.this, LoginActivity.class);
        finish();
        startActivity(intent);
    }
